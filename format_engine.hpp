...
namespace format_engine
{
    ...
    possible_types_variant get_value(const std::string &type, const std::any &any_)
    {
        ...
        if (type == "int")
            temp = std::any_cast<int>(any_);
        if (type == "float")
            temp = std::any_cast<float>(any_);
        if (type == "double")
            temp = std::any_cast<double>(any_);
        if (type == "std::string" || type == "string")
            temp = std::any_cast<const char *>(any_);
        if (type == "bool")
            temp = std::any_cast<bool>(any_);
    ...
    void extract_data(std::string &source,
                      std::vector<std::string> &types, std::vector<int> &declaration_order)
    {
    ...
    void substitute(std::string &source, std::stringstream &output_stream,
                    ...
                    const std::vector<std::any> &values)
    {
        ...
        {
            std::__throw_runtime_error("Format Engine: Expected more elements");
        }
        if (types.size() < values.size())
        {
            std::__throw_runtime_error("Format Engine: Too many elements provided");
        }
    ...
    std::string format(std::string &source, const std::vector<std::any> &values)
    {
        ...
        substitute(source, buffer_stream, types, declaration_order, values);
